...
func (s *AddURLService) Run(cmd *cobra.Command, args []string) error {
	...
	// Open pre-commit cache to update it (best-effort).
	cache, err := precommit_cache.Open(ctx)
	if err != nil {
		logger.Warn("pre-commit cache update skipped (open failed)", "error", err)
	} else {
		if err := cache.UpdatePathEntry(ctx, logger, input.path, oid, input.s3URL); err != nil {
