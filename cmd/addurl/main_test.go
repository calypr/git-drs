...
func TestRunAddURL_WritesPointerAndLFSObject(t *testing.T) {
	...
	cmds := [][]string{
		...
		{"config", "lfs.allowincompletepush", "true"},
	}
	...
	resetStubs := stubAddURLDeps(t, service,
		func(ctx context.Context, url string) (*cloud.ObjectMeta, error) {
			return &cloud.ObjectMeta{
				Platform:     cloud.PlatformS3,
				URL:          url,
				Bucket:       "bucket",
	...
	if tag := drsObject.Aliases[0]; tag != "git-drs-remote-url:true" {
		t.Fatalf("unexpected Aliases: %s", tag)
	}
...
func stubAddURLDeps(
	...
	inspectFn func(context.Context, string) (*cloud.ObjectMeta, error),
	isTrackedFn func(string) (bool, error),
) func() {
	t.Helper()
	origInspect := service.inspectBucketObject
	origIsTracked := service.isLFSTracked

	service.inspectBucketObject = inspectFn
	service.isLFSTracked = isTrackedFn

	return func() {
		service.inspectBucketObject = origInspect
...
func TestEnsureAllowIncompletePush(t *testing.T) {
	tests := []struct {
		name        string
		configValue string
		wantErr     bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Set the config value if specified
			if tt.configValue != "" {
				gitCmd(t, tempDir, "config", "lfs.allowincompletepush", tt.configValue)
			}

			// Test the function
			err := ensureAllowIncompletePush()

			if tt.wantErr {
				if err == nil {
					t.Errorf("ensureAllowIncompletePush() expected error, got nil")
					return
				}
				if tt.errContains != "" && !strings.Contains(err.Error(), tt.errContains) {
					t.Errorf("ensureAllowIncompletePush() error = %v, want error containing %v", err, tt.errContains)
				}
			} else {
				if err != nil {
					t.Errorf("ensureAllowIncompletePush() unexpected error = %v", err)
				}
			}
		})
	}
