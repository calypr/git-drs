...
This test deploys a small Calypr installation in a disposable kind cluster. It builds Syfon from the commit in `tests/integration/docker_syfon/syfon.ref` and git-drs from the current checkout. Real Fence tokens and Arborist policies authorize requests through Calypr revproxy.

The test runs git-drs's Gen3 remote test in a Kubernetes Job against the
...
Install Docker, kind, kubectl, Helm, Git, jq, and OpenSSL. Check out Syfon at the commit in `tests/integration/docker_syfon/syfon.ref` beside this repository, or set `TEST_SYFON_ROOT` to that checkout. The Docker server must use amd64 or arm64.
...
The command builds `syfon-calypr:ci` from the pinned Syfon checkout and `syfon-calypr-client:ci` from this git-drs checkout. The runner creates a cluster, runs every scenario, writes artifacts, and deletes the cluster.
...
Edit `values-ci.yaml` to update Fence, Arborist, revproxy, or Syfon image tags. Infrastructure image tags live beside their resources in `infrastructure.yaml` and `scenario-jobs.yaml`. Update `tests/integration/docker_syfon/syfon.ref` for a new Syfon commit; both backend and Gen3 CI use that pin. Update `versions.env` for a new Helm commit. The required Calypr components are:

- `calypr/gen3-helm` from `ohsu-develop`
- git-drs from the current checkout
...
- Syfon from the commit in `tests/integration/docker_syfon/syfon.ref`

The Fence and Arborist image tags are intentionally mutable because they match Calypr's deployment convention. The result and image artifacts record the tested Syfon and git-drs commits, image names, and runtime image IDs.
