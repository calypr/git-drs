...
syfon_root=${TEST_SYFON_ROOT:-$repo_root/../syfon}
SYFON_COMMIT=$(tr -d '[:space:]' <"$repo_root/tests/integration/docker_syfon/syfon.ref")
GIT_DRS_COMMIT=$(git -C "$repo_root" rev-parse HEAD)
...
preflight() {
  ...
  [[ "$SYFON_COMMIT" =~ ^[0-9a-f]{40}$ ]] || fail "syfon.ref must contain one lowercase 40-hex commit"
  test -f "$syfon_root/Dockerfile" || fail "Syfon checkout not found at $syfon_root"
  test "$(git -C "$syfon_root" rev-parse HEAD)" = "$SYFON_COMMIT" || fail "Syfon checkout must be at pinned commit $SYFON_COMMIT"
...
build_candidate() {
  ...
  docker build --tag "$SYFON_IMAGE" "$syfon_root"
}

build_client() {
  log "building git-drs client from current checkout at $GIT_DRS_COMMIT"
