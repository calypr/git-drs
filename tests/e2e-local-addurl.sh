...
# Compatibility wrapper for local add-url E2E.
# Uses localhost drs-server and local auth flow (optional basic auth).
...
export TEST_REPO_NAME="${TEST_REPO_NAME:-${REPO_NAME:-git-drs-e2e-addurl-local}}"
export TEST_WORK_ROOT="${TEST_WORK_ROOT:-${WORK_ROOT:-$(mktemp -d -t git-drs-e2e-addurl-local-XXXX)}}"
...
exec bash "$SCRIPT_DIR/e2e-gen3-remote-addurl.sh" "$@"
