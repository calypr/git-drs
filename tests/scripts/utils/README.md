...
This repository includes small helper scripts used for listing and cleaning DRS object records and bucket items.
...
- For `list-drs-sha256`, ensure you have postgres pod and postgres user credentials.
...
## Usage
```bash
./list-drs-sha256.sh <pod> <passwd> <resource> | ./delete-s3-by-sha256.sh <alias> <bucket>
./clean-drs-records.sh <pod> <passwd> <resource>
```